...
describe('MPesa Integration Test Suite', () => {
  ...
  beforeEach(() => {
    ...
    mockPrismaBooking.findMany.mockReset();
    mockAuth.mockReset();
  ...
  describe('API Route - Initiate Payment', () => {
    ...
    test('should validate the stored booking amount', async () => {
      mockAuth.mockResolvedValueOnce({ userId: 'user123' });
      ...
      expect(response.status).toBe(400);
      expect(data.error).toContain('Minimum payable service price');
    });
  ...
  describe('API Route - Callback Handler', () => {
    test('should process successful payment callback', async () => {
      ...
      mockPrismaBooking.findUnique.mockResolvedValueOnce({
        ...
        providerProfileId: 'provider123',
        providerProfile: { serviceCategory: 'Cleaning' },
      });
      mockPrismaBooking.findMany.mockResolvedValueOnce([]);
    ...
    test('should reject a callback with a conflicting account reference', async () => {
      const callbackData = {
        Body: {
          stkCallback: {
            MerchantRequestID: '12345',
            ...
            CallbackMetadata: {
              Item: [
                { Name: 'AccountReference', Value: 'BOOKING-other-booking' },
                { Name: 'Amount', Value: 100 },
              ],
            },
          },
      ...
      expect(response.status).toBe(400);
      expect(mockPrismaBooking.update).not.toHaveBeenCalled();
    });

    test('should not reprocess an already completed payment', async () => {
      const callbackData = {
        Body: {
          stkCallback: {
            MerchantRequestID: '12345',
      ...
      expect(response.status).toBe(200);
      expect(data.ResultDesc).toContain('already processed');
      expect(mockPrismaBooking.update).not.toHaveBeenCalled();
    });
